...
const AcceptFrndReQst = async(req, res)=>{
    const user  = req.user
    try {
        const sender = await FriendRequests.findOne({sender:user._id}).populate('sender', ['_id', 'name'])
        const newUser = await FriendRequests.findOne({receiver:sender.receiver._id}).populate('receiver', ['_id', 'name'])

        // console.log(newUser);
        if (newUser) {
            const frnd = await Friends.create({receiver:newUser})
            if (frnd) {
                const frndArr = await Users.findByIdAndUpdate(user._id, {$push:{friends:newUser._id}}, {new:true})
        console.log(frndArr);
